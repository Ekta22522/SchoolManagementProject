//
//  UpdateTeacherAssignmentView.swift
//  EduVerse360
//
...
struct UpdateTeacherAssignmentView: View {
    let assignmentId: Int

    @State var viewModel = UpdateTeacherAssignmentViewModel()
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading) {
                header
                if viewModel.isFetchingAssignment {
                    titleField
                    descriptionField
        ...
        // Runs once when the screen first appears — fetches the
        // assignment being edited plus the Class / Section menus.
        .task {
    ...
    private var submitButton: some View {
        Button {
            Task {
                await viewModel.updateTeacherAssignment(id: assignmentId)
            }
        } label: {
            Group {
...
#Preview {
    UpdateTeacherAssignmentView(assignmentId: 1)
}
